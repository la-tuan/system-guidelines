...
テスト設計書は、SLAや障害復旧マニュアルなどをテストベースとして作成します。  
システム障害を「サーバ障害」、「DB障害」、「ネットワーク障害」などに分け、停止、パフォーマンス劣化、など詳細化していき確認項目に落とし込みます。  

その他のテストを設計する観点として、下記を考慮します。

* 障害許容性（発生防止策）
  * イレギュラー処理の実装に関する工夫
  * 障害を事前に検知するためのトレース機能の工夫
  * 障害に対応するためのテスト環境準備に関する工夫
  * 障害管理（傾向、内容分析）に基づく、障害の予防・是正処置の実施
  * 運用時点での障害発生状況（傾向、内容分析）に基づく、障害の予防・是正処置の実施
  * ミスオペレーションの防止に関する工夫
  * システム資源（CPU、ディスクなど）の使用状況監視、警告に関する工夫
* 障害許容性（拡大防止策）
  * 停止防止対策
  * 稼働初期に起きる故障対策・分析に関する施策
  * 取り扱い可能なデータ件数等システムのキャパシティオーバー時の誤動作防止に関する工夫
  * 既存システム修正時における障害（リグレッションテスト漏れ、本番リリース不備による二次障害など）回避策
  * ハードウェアのアラーム対応に関する工夫
  * 障害対応マニュアルに関する工夫
  * 異常を検知し他システムなどへの影響を遮断する機能に関する工夫
  * 予防訓練に関する施策
* 回復性
  * 可用性（システムの稼働率、サービス提供時間、運転時間等の割合）向上施策
  * 障害分類を配慮した障害回復時間短縮施策
  * サービス提供再開施策（復旧予測時間の配慮など）
  * 自動リカバリー機能など故障耐久能力向上施策
  * バックアップ機への切り替えに関する施策
  * 広域・局所災害対策
  * 要件定義時点におけるユーザとベンダー間のコンテンジェンシープラン共有化

参考：[SEC BOOKS：高信頼化ソフトウェアのための開発手法ガイドブック](http://www.ipa.go.jp/files/000005144.pdf)
