...
- パスワードは一方向ハッシュを利用する。ハッシュの計算においてSaltを使用し、同一のパスワードであっても別の値になるよう実装すること。
...
- カード番号を平文のまま保管しないこと。アクセスログに記録されないよう、POSTメソッドで送信すること。
...
### 5.3 JSONPの利用について
- 原則としてJSONPは利用しないこと。どうしても利用が必要な場合は、プログラムソースの安全性をレビューし確認した上で、JSONP経由でアクセスさせるファイルの中の情報に機微なものが含まれないように実装すること。
...
- パスワードはsaltを利用して生成すること。saltは十分な長さ（生成されるハッシュ値の長さと同等かそれ以上）を持ち、かつユニークである必要がある。MACアドレスや上述のデバイストークンを使用することを推奨する。
...
- パスワードは絶対に平文で保存しない！
- iOSのキーチェインの中に、平文で機微な情報を保存しないこと。（iOSのキーチェインは怪しい）
...
- 利用したことをユーザが否認しないよう、ログを保存しておくこと。（署名済みの利用申請など）
- アプリケーションの利用如何によって、課金が発生する旨をユーザに告知すること。
- 購入に関するデータは、改ざんされない仕組みを構築すること。
- ホワイトリスト方式で、有料コンテンツを提供するようにすること。
- 例えば利用場所が突然全く異なる場所になったり、言語が明らかに変わった場合は再認証させるなどの仕組みを導入すること。
...
- 信頼できるライブラリかどうか、利用前にチェックを行うこと。社内で共有し、承認を取るように仕組み化しておくこと。
